...
labeled as `colliders.additional.csv`. If you look closely, one of the waypoints
called out in [LOCATIONS.md](LOCATIONS.md) is appearing as an easily accessible space in the map
and because of that, the drone will crash into a building when it goes there - 
...
homogeneous coordinates `(x, y, 1)` and to determine its determinant (no pun intended).
...
every three collinear neighbors. This is implemented in `prune_waypoints()`.
